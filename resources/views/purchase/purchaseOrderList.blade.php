<div class="table-head">
	...
	<table class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
		...
		<tbody>
			<tr ng-repeat="(key, value) in purchaseOrderListById | filter:{po_id: search.po_id, Pname: search.Pname, name: search.name, vname: search.vname, quantity: search.quantity, price: search.price}"> 
				<td>@{{value.po_id}}</td>
				<td>@{{value.Pname}}</td>
				<td>@{{value.name}}</td>
				<td>@{{value.vname}}</td>			
