<div class="table-head">
	<div class="row" style="position: relative; left: 280px;top: -15px;">
		<h3 class="head ">All Purchase Order Reject List</h3>
	...
	<table class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
		...
		<tbody>
			<tr ng-repeat="(key, value) in purchaseOrderRejectList | filter:{po_id: search.pr_id, Pname: search.Pname, name: search.name, quantity: search.quantity, price: search.price}"> 
				<td>@{{value.po_id}}</td>
				<td>@{{value.Pname}}</td>
				<td>@{{value.name}}</td>
