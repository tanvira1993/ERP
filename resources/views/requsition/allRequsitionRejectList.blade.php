...
<div class="row" style="position: relative; left: 280px;top: -15px;">
		<h3 class="head ">All Rejected Requsitions</h3>
	...
	<table class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
		...
		<tbody>
			<tr ng-repeat="(key, value) in prRejectList | filter:{pr_id: search.pr_id, Pname: search.Pname, name: search.name, quantity: search.quantity, type: search.type, requisition_name: search.requisition_name}"> 
				<td>@{{value.pr_id}}</td>
				<td>@{{value.Pname}}</td>
				<td>@{{value.name}}</td>			
