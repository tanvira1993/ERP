...
<div class="table-head">
	...
	<table class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
		...
		<tbody>
			<tr ng-repeat="(key, value) in materialReportOnMaterial | filter:{name: search.name, type: search.type, quantity: search.quantity, Pname: search.Pname,vname: search.vname}">
				<td>@{{key+1}}</td>
				<td>@{{value.name}}</td>
				<td>@{{value.type}}</td>			
