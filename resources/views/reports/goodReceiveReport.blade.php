	...
	<table class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
		<thead class="thead-dark">
			<tr>
				...
				<th class="th-sm" width="20%">Employer Name <br/><input ng-model="search.Pname" class="form-control input-sm" ></th>
				<th class="th-sm" width="20%">Vendor <br/><input ng-model="search.vname" class="form-control input-sm" ></th>

			</tr>
		</thead>
		<tbody>
			<tr ng-repeat="(key, value) in goodReceiveLists | filter:{name: search.name, type: search.type, quantity: search.quantity, Pname: search.Pname,vname: search.vname}">
				<td>@{{key+1}}</td>
				<td>@{{value.name}}</td>
				<td>@{{value.type}}</td>			
